...
namespace Nebula.Serialization
{
    public static class StringSerializer
    {
        ...
        private struct EnumJsonIntermediate
        {
            ...
            public static EnumJsonIntermediate FromEnum(Enum @enum)
            {
        ...
        public static bool CanSerializeType(Type type) => type.IsEnum || typeToHandler.ContainsKey(type);

        public static string Serialize(Type type, object value)
        {
            if(type.IsEnum)
            {
                return SerializeInternal(value, enumHandler);
            }
            if(typeToHandler.TryGetValue(type, out var typeHandler))
            {
                return SerializeInternal(value, typeHandler);
            }
            return string.Empty;

            string SerializeInternal(object value, SerializationHandler handler)
            {
                try
                {
        ...
        public static object Deserialize(Type type, string value)
        {
            if(type.IsEnum)
            {
                return DeserializeInternal(value, enumHandler);
            }
            if(typeToHandler.TryGetValue(type, out var typeHandler))
            {
                return DeserializeInternal(value, typeHandler);
            }
            return null;

            object DeserializeInternal(string val, SerializationHandler handler)
            {
                try
                {
                ...
                {
                    throw new Exception($"Error on string serializer", e);
                }
            }
        }
        ...
        static StringSerializer()
        {
            ...
            enumHandler = new SerializationHandler
            {
                serialize = (obj) => JsonUtility.ToJson(EnumJsonIntermediate.FromEnum((Enum)obj)),
                deserialize = (str) =>
                {
                    if (string.IsNullOrEmpty(str))
                        return default(Enum);
