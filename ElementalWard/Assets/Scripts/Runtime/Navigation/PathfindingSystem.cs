...
namespace ElementalWard.Navigation
{
    public static class PathfindingStatic
    {
        ...
        private static List<Vector3> path = new();
        private static NativeArray<int2> _neighbourOffsets;
        private static float stopwatch;

        static PathfindingStatic()
        {
            ...
            ElementalWardApplication.OnShutdown += ElementalWardApplication_OnShutdown;
        }

        private static void ElementalWardApplication_OnShutdown()
        {
            _neighbourOffsets.Dispose();
            ElementalWardApplication.OnShutdown -= ElementalWardApplication_OnShutdown;
        }

        public static FindPathJob RequestPath(AStarNodeGrid aStarNodeGrid, Vector3 start, Vector3? end, float actorHeight, float actorRadius, float actorJumpStrength)
        {
